...
@app.route('/index')
def index():
    imgs = ['ap1.png', 'ap2.png']
    subjs = ["Default apartments", "Studies", "Loft", "Luxury apartments"]
    ...
    return render_template('index.html', title='Apartments masters', pname='HOME', navmenu=controller.navmenu,
                           imgs=imgs, subjs=subjs, processed_files=processed_files)

# Обработка запроса к странице contact.html
@app.route('/contact')
def contact():
    return render_template('contact.html', title='Apartments masters', pname='CONTACT', navmenu=controller.navmenu)
